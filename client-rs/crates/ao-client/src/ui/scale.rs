...
pub fn resolve(logical_size: Vec2, device_pixel_ratio: f32) -> ScaleDomains {
    ...
    ScaleDomains { device, world: world_scale(logical_size), ui: ui_scale(logical_size) }
}

/// Logical width above which the world is drawn at double size.
///
/// A tile is 32 source pixels. On a very large window that is a small fraction
/// of the screen, and doubling keeps the art readable without showing more of
/// the map than the roadmap's visibility bound allows.
const WORLD_ZOOM_2X_WIDTH: f32 = 2560.0;

/// Whole logical pixels per world pixel.
///
/// A function of the window's logical size, deliberately *not* of the device
...
mod tests {
    ...
    fn a_device_pixel_ratio_change_alone_does_not_change_framing() {
        ...
        let baseline = visible_tiles(logical, resolve(logical, 1.0));

        for ratio in RATIOS {
            let domains = resolve(logical, ratio);
            assert_eq!(
    ...
    fn a_device_pixel_ratio_change_alone_does_not_move_pointer_targets() {
        ...
        // sharper monitor.
        let logical = Vec2::new(1280.0, 720.0);
        let baseline = resolve(logical, 1.0).tile_logical_px();

        for ratio in RATIOS {
            assert_eq!(
                resolve(logical, ratio).tile_logical_px(),
                baseline,
    ...
    fn a_sharper_display_renders_the_same_view_with_more_physical_pixels() {
        ...
        let logical = Vec2::new(1280.0, 720.0);
        let one = resolve(logical, 1.0);
        let two = resolve(logical, 2.0);
    ...
    fn the_world_zoom_is_always_a_whole_number_of_logical_pixels() {
        ...
        for width in [640.0, 1280.0, 1920.0, 2560.0, 3840.0, 7680.0] {
            let domains = resolve(Vec2::new(width, 900.0), 1.0);
    ...
    fn the_world_zoom_follows_the_window_and_not_the_display() {
        // Two ratios, same window: identical zoom. Same ratio, much larger
        // window: a larger zoom, because 32 source pixels is a small fraction
        // of a very wide screen.
        let small = Vec2::new(1280.0, 720.0);
        let large = Vec2::new(3840.0, 2160.0);

        assert_eq!(resolve(small, 1.0).world, resolve(small, 2.0).world);
        assert!(resolve(large, 1.0).world > resolve(small, 1.0).world);
    }
    ...
    fn snapping_is_idempotent() {
        ...
        // not drift.
        let domains = resolve(Vec2::new(1280.0, 720.0), 1.0);
