...
# What the client should be claiming. See scripts/build-stamp.sh.
build_stamp_expected() {
  bash scripts/build-stamp.sh
}
...
case "$TARGET" in
  ...
  check)
    ...
    # The world corpus against its pinned baseline. The pack is generated by the server and
    # is not in the repository, so this runs when it is there and says so when it is not —
    # a check that quietly passes on a machine without the data is worse than no check,
    # because it reads as verification.
    echo "==> world topology baseline"
    pack="$(ls ../client/dist/data/packs/maps.*.pack ../client/public/data/packs/maps.*.pack 2>/dev/null | head -1 || true)"
    if [ -n "$pack" ]; then
      cargo run --quiet --release -p ao-topology -- --check "$pack" | sed 's/^/    /'
    else
