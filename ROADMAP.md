...
Tasks `1-49` are the backend parity path. Tasks `50-59` are security
and remaining parity-drift hardening and should be closed before calling the
backend hardened. Tasks `60-88` are backend modernization, operations, and
testability refactors. Tasks `89-90` are optional legacy features that the
inspected VB6 baseline kept disabled and should only be revisited after backend
modernization unless a target shard explicitly re-enables them. Tasks `91-200`
are browser proof, frontend product, and UX work, intentionally placed at the
end so backend and security work stay first.
...
- If the inspected VB6 baseline disables a feature, parity only requires
  ...
  that feature belongs after backend modernization unless the selected shard
  explicitly requires it earlier; any browser UI for it belongs in the final
...
- **Tasks 47-49:** do serially. These are scope decisions and final drift
  cleanup.
- **Tasks 50-59:** do before frontend/product work. These are security and
  remaining parity-drift hardening tasks.
- **Tasks 60-88:** parallelize by subsystem after the state-helper/testability
  seam work is isolated. These are backend modernization, operations, and
  testability tasks and should not block parity signoff unless explicitly
  promoted.
- **Tasks 89-90:** do only after backend modernization unless the target shard
  explicitly promotes them back into the parity path.
- **Tasks 91-200:** intentionally last. Split by browser proof, frontend UX,
  account/lobby, i18n, multi-realm, and release-hardening surfaces.
...
50. Make party invites authoritative and leader-only.
...
53. Finish secure trade start validation to match the inspected VB6 safety
     rules.
...
Testability order: state helpers first, then pure combat/progression math, then
shared death resolution, then focused spell/NPC-AI seams. Do not start with a
full `{state, effects}` rewrite, do not split `process_regen_tick` only because
it is long, and do not abstract occupancy beyond targeted helper improvements
unless a concrete bug or test seam requires it.
...
60. Replace NPC aggro full scans with spatial-grid queries.
     ...
     full player count.
61. Replace pet target full scans with bounded or indexed lookup.
...
63. Add per-MapServer hotspot telemetry.
     Outcome: player count, NPC count, tick duration, mailbox length, and
     broadcast rates are visible per map.
64. Add `Arena.Map.State` update helpers: `put_player/3`, `update_player/3`,
...
65. Extract pure combat and progression math from side-effectful handlers.
     Pull out testable functions such as `Combat.resolve_spell_damage/3`,
     `Combat.resolve_melee_damage/3`, `Progression.check_level_up/1`, and
     `Progression.calculate_xp_award/3` from the large handler paths without
     changing gameplay behavior.
     Outcome: core damage, hit, XP, and level-up rules can be unit-tested
     directly without MapServer setup or packet-send side effects.
66. Consolidate NPC and player death resolution behind shared helpers.
     Replace duplicated death cleanup in spell damage, melee attack, pet AI,
     NPC AI, and poison/buff ticks with shared `resolve_npc_death` and
     `resolve_player_death` paths.
     Outcome: occupancy cleanup, removal broadcasts, loot, XP, quest progress,
     guild/faction rewards, and death side effects are tested in one place.
67. Split `apply_spell_damage` into small request, damage, death, and broadcast
     helpers.
     Keep spell behavior unchanged, but separate target detection, resistance,
     damage application, NPC/pet/player death handling, XP/reward updates, and
     packet broadcasts.
     Outcome: the largest spell path stops mixing all concerns in one function
     and individual branches become directly testable.
68. Make NPC AI return effects where it materially improves tests.
     Start with NPC AI paths that currently inline packet sends; return
     `{state, effects}` or an equivalent local effect structure from those AI
     functions and execute effects at the boundary.
     Outcome: NPC AI behavior can be unit-tested without process mailboxes,
     while avoiding a full handler-wide effects rewrite.
69. Split `gm_commands.ex` (1,657 lines) by command family: `Gm.Moderation`,
     ...
     centralized.
70. Introduce handler effects only where they remove real coupling: GM audit
...
71. Implement the authoritative persistence and backend refactor plan from
     [research/arena-authoritative-persistence-and-refactor.md](research/arena-authoritative-persistence-and-refactor.md).
...
72. Pre-resolve `.dat` references at load time where hot-path lookups still
     repeat.
     Outcome: gameplay avoids repeated definition lookups that can be resolved
     once at startup.
73. Unify interest management for players, NPCs, and ground items.
...
81. Add deployment pipeline and backup/restore runbook.
     Outcome: releases and recovery have a documented path.
82. Add TLS for HTTPS and WSS.
     Outcome: production browser/session traffic is encrypted.
83. Add asset CDN/delivery strategy for static resources.
     Outcome: heavy client assets do not depend on the gameplay server path.
84. Add automated backups and database connection-pool tuning.
     Outcome: the database operational path is production-safe.
85. Add runtime-tunable settings for intervals, rates, and formula constants.
...
87. Add pre-public scripted load/soak runs.
...
89. If the selected shard explicitly re-enables clan relations, implement the
...
90. If the selected shard explicitly re-enables guild elections, implement the
...
163. Stop using socket `login_new_char` as the primary browser account flow. DONE.
     Outcome: account auth and gameplay auth are clearly separated.
164. Launch the AO socket with `login_existing_char(char_id, session_token)`. DONE.
     Outcome: gameplay protocol stays unchanged after the HTTP lobby.
165. Prefer same-origin serving or proxying for the account API. DONE.
...
188. Add shared client-vs-`ao_protocol` contract tests for browser packet
     shapes.
...
189. Add deterministic harness/demo routes or fixtures for the critical
     browser UI states.
     ...
     manual setup.
190. Add asset/map-pack/cache failure handling and explicit fallback UI.
...
191. Expand reconnect and partial-bootstrap recovery behavior and browser
     tests.
     ...
     client state.
192. Add an explicit Playwright snapshot/update discipline and release check
...
196. Define the fast frontend unit-test lane and keep it separate from browser
...
197. Add accessibility and keyboard coverage for the core browser panels.
...
198. Define asset/map-pack/browser-cache versioning and invalidation rules.
...
199. Split frontend CI into a fast unit/protocol lane and a slower browser/
...
200. Make the authoritative-vs-inferred browser UI rule explicit and test it.
...
- **Recommended frontend priority:**
  - Fast tests and build gates.
  - Frontend-only parity and UX.
  - Protocol/reducer/visual test depth.
  - Measured rendering/state performance work.
  - Backend-authoritative social/UI.
  - Account/lobby/i18n/multi-realm product work.
...
- Asset/cache failure handling.
- Explicit "frontend parity done" criteria.
