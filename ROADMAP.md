...
This is the repository's single active execution plan. The Rust/Bevy client targets
browser/WASM and native desktop from one Rust codebase, and its server, protocol,
content, operations and release dependencies live in this same sequence. It answers
...
- [`CHANGELOG.md`](CHANGELOG.md) owns new completed work and dated evidence.
...
would otherwise be unclear; it does not create a separate queue or permit one side of
a cross-stack acceptance test to close without the other.
...
code that owns the meaning — `client-rs/crates/ao-core/fixtures/tile_semantics.txt`
against `Arena.Map.CsmParser` and
...
- `closed`: all task and applicable phase gates passed; new closures live only
  in the root changelog, while the pre-consolidation client archive stays
  read-only.
...
Define the corresponding server measurement vocabulary now—movement RTT,
MapServer job duration, queue depth, memory per connection, login throughput and
autosave latency—without inventing release thresholds before W-0108 produces a
representative high-load baseline. These are job and service budgets, never
permission to introduce a global simulation tick.

### Phase 1 exit gate

- [ ] `client-rs/build.sh check` passes for WASM, both Rust crates and the roadmap gate.
...
Cover existing-account login, registration, character creation, launch and the
first authoritative gameplay/reconnect-ready state through live-backend browser
E2E. Implement the Google authentication endpoint plus Google-only and linked-
account flows. Add manual stat allocation during character creation only if its
product scope is explicitly retained; otherwise record the rejection rather than
leaving an ambiguous fixture-only control.
...
Keep built-in minimap/world markers unless product evidence authorizes custom
user or quest markers. If authorized, specify their persistence, visibility,
moderation and server-validation boundary before implementation.
...
Prove party, guild and faction streams plus service/system channel separation
against the live backend, not only fixtures or decoded packet units.
...
Drive the party and clan panels from authoritative snapshots. Live-backend E2E
covers party membership/roster updates and clan details, news, online state and
permission changes across reconnect; fixture gallery state cannot satisfy this
task.
...
parameters. Define locale metadata and extraction, and audit every remaining
hard-coded player-facing server string. Modern protocol outcomes use a stable
semantic event/reason key, typed parameters, severity, presentation channel,
accessibility-announcement behavior and a safe unknown-key fallback. Localize in
the client; retain server-authored prose only behind an explicitly tested legacy
adapter.
...
data without passwords or startup failure. Locale preference is part of the
versioned settings contract and falls back deterministically when a locale is
...
Gate release-critical controls and outcomes on screen-reader names and
announcements, automated color contrast and reduced-motion behavior. Document
the supported responsive/mobile policy rather than implying touch support from
a browser build alone.
...
The seamless Rust/WASM product path is intentionally ahead of this section. These
repository-wide server obligations are not a second queue: they are stable tasks in the
same execution sequence, reached after the authoritative client, gameplay and world-fidelity
work they measure or protect. A blocked external capture task does not stop later
dependency-ready work.

### Task W-0106 — Confirm and close the remaining stun parity drift [server]

- **State:** research-gated
- **Phase:** 10
- **Depends on:** confirmed VB6 source or formula evidence

Resolve the only remaining item from the former deterministic-parity phase:
`:stun_start` and melee stun-on-hit behavior. Locate authoritative VB6 source/formula
evidence, pin the exact trigger, duration, immunity and emitted packet semantics, then wire
the missing behavior and shrink `drift.md`. Do not infer combat rules from names or from the
current port. If the source cannot be recovered, retain the explicit research gate rather
than inventing parity.

### Task W-0107 — Deterministic server time and scheduling [server]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0096

Provide an injectable clock and deterministic scheduling helpers for cooldowns, buffs,
autosave, combat, periodic map jobs and transfer timing. Replace parity-gate
`Process.sleep/1` dependence wherever controlled time can prove the same behavior. Preserve
event-driven MapServer simulation: this is test control, not a global game tick, regional
barrier or protocol clock.

### Task W-0108 — High-load benchmark and regression budgets [shared]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0020, W-0028, W-0107

Build a repeatable high-load bot benchmark and versioned baselines on representative
hardware. Gate movement RTT p99, NPC-AI/regen/buff job duration p99, queue depth, memory per
connected player, login throughput and autosave latency; report absolute values and change
from the accepted baseline. Keep the exact parity-gate suites in
`server/docs/PARITY_GATE.md` and distinguish a scheduler/job budget from simulation time.

### Task W-0109 — Long-duration gameplay and transition soak [shared]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0099, W-0108

Run ten-minute-or-longer scripted soaks with memory, process, scheduler and queue-depth
tracking. Include combat/loot/death/resurrection and repeated map/seam transitions. Assert
no duplicate entity, stale session, abandoned transfer, monotonic heap growth or unbounded
queue; publish pass/fail thresholds and the measured result.

### Task W-0110 — VB6 capture corpus and authoritative packet replay [shared]

- **State:** blocked
- **Phase:** 10
- **Depends on:** W-0028

Build and version a real VB6 packet-capture corpus once a Windows/VB6 capture environment is
available. Replay login, creation, bootstrap, movement, transfer, chat, services, inventory,
equip/use, combat, spells, death, bank, trade, party, guild, faction, reconnect and logout.
Until that external environment exists, keep byte fixtures and deterministic Rust/Elixir
traces as the executable substitute and do not claim they are a real-client capture.

### Task W-0111 — Concurrent combat integration proof [server]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0040, W-0107

Drive multiple live TCP clients against the same NPC and, where PvP rules permit, the same
player. Prove damage ordering, XP, loot, death, visibility and persistence stay consistent
under concurrency and that retries or delayed messages cannot double-apply an outcome.

### Task W-0112 — Guild persistence integration proof [server]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0044, W-0107

Create a guild through the live flow, restart or reload persistence, and verify guild state,
membership, rank, permissions, news and chat eligibility survive. Cover partial failure,
retry and concurrent membership mutation without hiding DB ownership errors behind timing.

### Task W-0113 — NPC commerce persistence integration proof [server]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0043, W-0107

Buy and sell through the TCP-level flow, verify authoritative gold and inventory mutations,
then reconnect and prove persistence. Exercise rejection, insufficient funds/capacity,
duplicate commands and mid-operation disconnect without duplicating items or currency.

### Task W-0114 — Reproducible failure bundles [shared]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0028, W-0108

For integration, replay, load and soak failures, emit a bounded bundle containing RNG seed,
deterministic-clock state, bot scenario/action history, redacted packet trace, state snapshots,
first meaningful diff, logs, metrics, commit and runtime configuration. Prove secrets and
private chat/account data cannot enter the bundle.

### Task W-0115 — Server telemetry, dashboards and alerts [server]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0099, W-0108

Finish telemetry for authentication failures, launch-credential use, map lifecycle,
handoffs, queue pressure, persistence, combat and cross-region work. Publish metrics and
dashboards for saturation, latency, errors and capacity, with actionable alerts that name
runbooks and avoid high-cardinality entity/player labels.

At minimum, preserve the former server plan's named signals: MapServer/backpressure queue
depth, disconnect reasons, `send_pend`, autosave-task failures, egress shed/coalesce counts,
launch-credential issue/consume/expiry/replay/revocation, protocol-version/capability mix,
authentication-to-bootstrap latency/failure and command-receipt rejection/timeout/stale-
version counts. Alert on sustained shedding, forced backpressure disconnects, launch-
credential abuse and bootstrap latency/failure budget violations.

### Task W-0116 — Runtime administration and safe operator actions [server]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0115

Provide authenticated, audited runtime inspection, account/character/session lookup and
bounded administrative actions. Every mutation has authorization, reason, target, before/
after result, idempotency or explicit retry behavior; no tool bypasses MapServer authority,
topology versions or persistence ordering.

Cover map and BEAM-process inspection, map control, online-player lookup, moderation/world
actions, log inspection and health checks. Read-only inspection and mutation are distinct
permissions, and a failed or partially applied action reports its authoritative outcome.

### Task W-0117 — Incident runbooks, durable audit and SLOs [server]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0115, W-0116

Write executable runbooks for authentication, database, world, queue, handoff, asset and
deployment incidents. Store moderation/security audit events durably with review/export,
retention and redaction controls. Define service-level objectives and error budgets for
login, gameplay, persistence and seamless transitions, and connect alerts to those budgets.

Audit storage is append-only as the system of record, tamper-evident, RBAC-protected and
searchable by actor, target, action and time range. Runbooks explicitly cover database
outage, MapServer crash, gateway overload and deploy rollback. SLOs include availability,
login success, movement latency, disconnect rate, persistence failure, recovery-time and
acceptable-data-loss objectives, and their error budgets participate in release decisions.

### Task W-0118 — Executable abuse and anti-cheat gate [server]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0023, W-0040, W-0043

Keep the exploit/parity audit executable and expand adversarial coverage for malformed
protocol input, impossible movement, combat, chat, economy and command replay. Harden rate
and authority checks without making the WASM client an anti-cheat boundary or silently
breaking retained VB6/unframed compatibility.

### Task W-0119 — Account and game-launch security lifecycle [shared]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0025, W-0027

Complete password reset, email verification, session listing/revocation, throttling,
lockout policy, administrative MFA, secret/signing-key rotation, recovery and deletion.
Adversarially test launch credential issuance and WebSocket authentication for theft,
expiry, replay, revocation, wrong character, malformed/concurrent consume and enumeration.
Redact every credential/session/recovery secret and enforce the production HTTPS/WSS and
origin policy owned by W-0075.

Rate-limit issuance and socket authentication by account, IP, character and justified risk
signal without creating an account-enumeration oracle. Exercise emergency global/session
revocation and key rotation while live sessions and audit exports remain safe.

### Task W-0120 — Indexed local NPC and pet targeting [server]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0102, W-0108

Replace NPC aggro and pet-target full-map scans with spatial-grid or other bounded local
queries. Reuse the authoritative interest model for players, NPCs and ground items without
routing local work through a global process. Close only with crowded-map allocation and
latency evidence proving behavior, ordering and target choice remain correct.

### Task W-0121 — Focused guild-server modules [server]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0112

Split `guild_server.ex` into focused ownership, persistence, membership, permissions and
communication modules while preserving one explicit mutation order. Prove restart,
concurrency and chat-eligibility behavior through the integration evidence from W-0112.

### Task W-0122 — Pre-resolved data and runtime tuning [server]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0108

Pre-resolve `.dat` references where profiling finds repeated hot-path lookups. Introduce
validated runtime-tunable intervals, rates and formula constants with safe defaults,
bounded ranges, auditability and deterministic test configuration. Do not turn a tunable
interval into a global simulation tick.

### Task W-0123 — Explicit persistence ownership and failure semantics [server]

- **State:** planned
- **Phase:** 10
- **Depends on:** W-0100, W-0112, W-0113

Finish the long-term persistence boundary: one explicit character write path, one guild
write path, and named ordering, retry, idempotency and failure semantics. Keep guild writes
synchronous until a separate user-experience and consistency design is approved. Prove
autosave, logout, reconnect, migration and crash recovery cannot overwrite newer state.

### Task W-0124 — Crowded-map NPC-AI allocation experiment [server]

- **State:** research-gated
- **Phase:** 10
- **Depends on:** W-0108

Measure `effects ++ [effect]` and repeated collection rebuilding in `npc_ai.ex` under a
crowded representative map. Replace it with prepend/reverse or another accumulator and reuse
static data only if before/after allocation and tick-latency results show meaningful gain.
The current 50-player regen/buff measurements are already far inside budget, so code shape
alone does not authorize a rewrite.
...
Define immutable long-lived caching for content-hashed WASM, packs, indices,
sprites and audio; short-lived revalidation for manifests and entry points; and
explicit CDN range, integrity, CORS, purge and rollback policy. Production
rejects insecure transport and unexpected origins rather than silently
downgrading.
...
Include the pre-public load/soak result from W-0109 as a release input with an
explicit pass/fail threshold.

Keep deterministic visual baselines for the world, in-world labels, NPCs,
objects, inventory, bank and trade surfaces. Exercise stale world packs, missing
asset indices, changed client-build hashes and reload/retry recovery, plus the
documented low-bandwidth/degraded-network profile. Formatting, clippy, native,
WASM, shared packet contracts, fuzz/model tests and production build fail before
browser E2E when their lower-layer contract is already broken.
...
Produce automated server/client release artifacts and deployment, backup and
restore automation. Tune database pools, define live migration while the
previous release remains available, drain sessions gracefully and prove
rollback after failed application and database changes. Restore into a clean
environment and verify account, character, guild and bank data before public
release, recording the drill result. Stage the previous supported Rust/Bevy
client against the candidate server and, where supported, the candidate client
against the previous server before rolling deployment.
...
Pin Elixir, Erlang/OTP, Hex, Rust, Cargo, clippy, rustfmt, wasm-bindgen, browser
harness and Nix versions, with documented Nix and non-Nix paths. Add one
development verification command that checks those tools and map-pack
prerequisites with actionable errors. Distinguish Nix/Rust, wasm-bindgen,
asset, pack and browser-harness build failures while keeping the shipped build
stamp tied to exact clean source. Validate broken exits, unreachable spawns,
duplicate or invalid object/NPC/spell/recipe/drop references, missing map
triggers needed for presentation such as roof hiding, and world-pack/client-
build content-hash compatibility before admitting a release artifact. Every
failure names the offending record and corrective command.
...
- optional legacy guild alliances/peace proposals and guild elections only
  after explicit product approval and a maintained-protocol owner exist;
- regional realm support—realm selection in backend and Rust/Bevy, monitoring,
  markets/character authority and controlled transfer policy—only after a
  falsifiable need is demonstrated;
...
Maintain source-backed briefs from official AO documentation/source, player and
support feedback, comparable MMORPGs, internal prototypes and production
telemetry. Every serious candidate names the player problem and audience,
smallest useful experiment, reusable dependencies, engineering/content/
moderation/operating cost, economy/balance/abuse/pay-to-win risks and measurable
success criteria. Review candidates consistently for player value, retention,
social coordination, leverage per developer hour, architecture fit,
accessibility, safety and ongoing cost; record an explicit accept, defer or
reject outcome. Supporter ideas are assessed separately from gameplay access,
with worlds, quests, destinations, classes and competitive power kept free.
...
## Roadmap consolidation ledger

This ledger is an omission check, not another execution or priority list. On
2026-08-23 the former root server and nested client roadmaps were merged
into this file. Existing cross-stack tasks retained their stable IDs; root-only
obligations received new IDs in the same sequence; completed work stayed in
history rather than being reopened.

| Former root scope | Canonical owner after consolidation |
| --- | --- |
| Remaining `:stun_start` drift | W-0106 |
| Deterministic clocks and scheduling | W-0107 |
| High-load benchmark, parity suites and performance budgets | W-0020, W-0108 |
| Long, combat and map-transition soak | W-0109 |
| Real VB6 captures and whole-protocol replay | W-0028, W-0110 |
| Concurrent combat, guild persistence and NPC commerce integration | W-0111, W-0112, W-0113 |
...
| Character/guild persistence boundary | W-0100, W-0123 |
| Empty-map timer lifecycle | Completed in root `CHANGELOG.md` (`719536ce`, `af8ec36e`) |
| Crowded-map NPC-AI allocation hypothesis | W-0124 |
| Production topology, AOI, navigation and instances | W-0101, W-0095, W-0102, W-0103, W-0104 |
| Release artifacts, deployment, TLS/CDN, backup/restore, migrations, staging and rollback | W-0075, W-0077, W-0079, W-0080, W-0081 |
| Toolchains, developer verification, build diagnostics and data validation | W-0082 |
| Live Rust/Bevy product, workflows, semantic events and accessibility | W-0025, W-0036, W-0037, W-0038, W-0039, W-0040, W-0041, W-0042, W-0043, W-0044, W-0045, W-0046, W-0047, W-0048, W-0069, W-0071, W-0077 |
| Optional guild relations/elections, realms and future-product research | W-0083 |
...
- `bash scripts/check_roadmap.sh` passes, and completed task bodies and dated
  evidence move to the root `CHANGELOG.md`.
