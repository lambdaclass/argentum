defmodule Arena.Map.GmCommands do
  ...
  Public surface returns `{:ok, state, [Effect.t()]}`. Every sub-handler
  family module (`Arena.Map.Gm.*`) speaks the same effects contract, so
  the router simply delegates and surfaces the produced tuple unchanged.
  """
